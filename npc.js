...
async function openWorldSetup(interaction) {
  ...
  return interaction.reply({
    embeds: [
      new EmbedBuilder()
        .setTitle("World Setup v5 • Workplace Settings")
        .setDescription(
...
export async function handleNpcComponent(interaction) {
  ...
  // Unknown Location Inbox / Classify buttons.
  // Supports npc_world:classify:<id>, npc_world:unknown_classify:<id>,
  // and other npc_world button IDs containing "classify".
  if (
    interaction.isButton() &&
    id.startsWith(`${WORLD_PREFIX}:`) &&
    id.toLowerCase().includes("classify")
  ) {
    return openClassificationFromButton(interaction);
  }
  ...
  if (interaction.isButton() && id.startsWith(`${WORLD_PREFIX}:work_yes:`)) {
    const locationId = id.split(":").pop();
    return setWorkplaceEligibility(interaction, locationId, true);
  }

  if (interaction.isButton() && id.startsWith(`${WORLD_PREFIX}:work_no:`)) {
    const locationId = id.split(":").pop();
    return setWorkplaceEligibility(interaction, locationId, false);
  }

  if (interaction.isButton() && id.startsWith(`${WORLD_PREFIX}:work_details:`)) {
    const locationId = id.split(":").pop();
    return openWorkplaceDetailsModal(interaction, locationId);
  }

  if (interaction.isModalSubmit() && id.startsWith(`${WORLD_PREFIX}:work_details_modal:`)) {
    const locationId = id.split(":").pop();
    return saveWorkplaceDetails(interaction, locationId);
  }
}
...
async function editWorldLocationFromDiscordChannel(interaction) {
  ...
  let { data: location, error } = await supabase
    .from("npc_world_locations")
    .select("id,name,location_type,discord_object_type,custom_type_name,custom_type_description,is_custom_type,can_npcs_work_here,workplace_details")
    .eq("guild_id", interaction.guildId)
...
async function openClassificationFromButton(interaction) {
  // Acknowledge immediately so Discord never times the Classify click out
  // while Supabase/channel lookups are running.
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });

  const id = interaction.customId || "";
  const token = id.split(":").pop();

  if (!token || token === "classify" || token === "unknown_classify") {
    return interaction.editReply({
      content:
        "I received the Classify click, but this button does not contain a location/channel ID. " +
        "Run `/npc world setup` → **Edit Location** once to refresh the location controls.",
    });
  }

  // First treat the token as an npc_world_locations row ID.
  let { data: location, error } = await supabase
    .from("npc_world_locations")
    .select("id,name,location_type,discord_object_type,discord_channel_id,custom_type_name,custom_type_description,is_custom_type")
    .eq("guild_id", interaction.guildId)
    .eq("id", token)
    .maybeSingle();

  if (error) {
    console.error("[LifeLine] Classify lookup by location ID failed:", error.message);
  }

  // If it wasn't a DB row ID, treat it as a Discord channel/category ID.
  if (!location) {
    const byChannel = await supabase
      .from("npc_world_locations")
      .select("id,name,location_type,discord_object_type,discord_channel_id,custom_type_name,custom_type_description,is_custom_type")
      .eq("guild_id", interaction.guildId)
      .eq("discord_channel_id", token)
      .maybeSingle();

    if (byChannel.error) {
      console.error("[LifeLine] Classify lookup by channel ID failed:", byChannel.error.message);
    } else {
      location = byChannel.data;
    }
  }

  // If the button points to a Discord channel not yet in the registry, create it.
  if (!location) {
    const channel =
      interaction.guild.channels.cache.get(token) ||
      await interaction.guild.channels.fetch(token).catch(() => null);

    if (!channel) {
      return interaction.editReply({
        content:
          "I received the Classify click, but I couldn't match it to a saved Life Line location " +
          "or a Discord channel/category.",
      });
    }
    ...
    if (inserted.error) {
      return interaction.editReply({
        content: `I received the Classify click, but couldn't save that location: ${inserted.error.message}`,
      });
    }

    location = inserted.data;
  }
...
function workplaceRows(locationId) {
  const yes = new ButtonBuilder()
    .setCustomId(`${WORLD_PREFIX}:work_yes:${locationId}`)
    .setLabel("NPCs Can Work Here")
    .setEmoji("💼")
    .setStyle(ButtonStyle.Success);

  const no = new ButtonBuilder()
    .setCustomId(`${WORLD_PREFIX}:work_no:${locationId}`)
    .setLabel("Not a Workplace")
    .setEmoji("🚫")
    .setStyle(ButtonStyle.Secondary);

  const details = new ButtonBuilder()
    .setCustomId(`${WORLD_PREFIX}:work_details:${locationId}`)
    .setLabel("Workplace Details")
    .setEmoji("📝")
    .setStyle(ButtonStyle.Primary);
...
async function setWorkplaceEligibility(interaction, locationId, canWorkHere) {
  await interaction.deferUpdate();

  const { data, error } = await supabase
    .from("npc_world_locations")
    .update({
      can_npcs_work_here: canWorkHere,
    })
    .eq("guild_id", interaction.guildId)
    .eq("id", locationId)
    .select("id,name,can_npcs_work_here,workplace_details")
    .single();

  if (error) {
    return interaction.editReply({
      content: `Could not update workplace setting: ${error.message}`,
      components: [],
    });
  }

  const status = canWorkHere
    ? "💼 NPCs **can work here**."
    : "🚫 This location is **not a workplace**.";

  return interaction.editReply({
    content:
      `✅ Updated **${data.name}**.\n\n${status}\n\n` +
      (canWorkHere
        ? "You can also add optional workplace details, such as the kinds of jobs normally found here."
        : "Life Line will not assign NPC jobs to this location unless you change this setting."),
    components: canWorkHere ? workplaceRows(locationId) : [],
  });
}

async function openWorkplaceDetailsModal(interaction, locationId) {
  const modal = new ModalBuilder()
    .setCustomId(`${WORLD_PREFIX}:work_details_modal:${locationId}`)
    .setTitle("Workplace Details");

  const details = new TextInputBuilder()
    .setCustomId("workplace_details")
    .setLabel("Workplace details (optional)")
    .setPlaceholder("Pet groomers, receptionist, manager, vet techs...")
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(false)
    .setMaxLength(1000);
...
async function saveWorkplaceDetails(interaction, locationId) {
  await interaction.deferReply({ flags: MessageFlags.Ephemeral });

  const workplaceDetails = interaction.fields
    .getTextInputValue("workplace_details")
    .trim();

  const { data, error } = await supabase
    .from("npc_world_locations")
    .update({
      workplace_details: workplaceDetails || null,
      can_npcs_work_here: true,
    })
    .eq("guild_id", interaction.guildId)
    .eq("id", locationId)
    .select("id,name,workplace_details,can_npcs_work_here")
    .single();

  if (error) {
    return interaction.editReply({
      content: `Could not save workplace details: ${error.message}`,
    });
  }

  return interaction.editReply({
    content:
      `✅ Workplace details saved for **${data.name}**.\n\n` +
      `💼 NPCs can work here.` +
      (data.workplace_details
        ? `\n\n**Workplace details:** ${data.workplace_details}`
        : ""),
  });
}
...
async function saveWorldLocationType(interaction, locationId, newType) {
  ...
  return interaction.editReply({
    content:
      ...
      `**Can NPCs work at this location?**`,
    components: workplaceRows(locationId),
  });
}
...
async function reviewWorld(interaction) {
  ...
  const { data, error } = await supabase
    .from("npc_world_locations")
    .select("id,name,location_type,discord_object_type,admin_confirmed,custom_type_name,is_custom_type,can_npcs_work_here")
    .eq("guild_id", interaction.guildId)
    .order("name", { ascending: true })
    .limit(25);
  ...
  const lines = data.map((loc) => {
    ...
    return `• **${loc.name}** → ${displayType}${loc.admin_confirmed ? " ✅" : ""}${loc.can_npcs_work_here ? " • 💼 Workplace" : ""}`;
  });
