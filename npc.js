...
async function openWorldLocationPicker(interaction) {
  ...
  return interaction.reply({
    content:
      "Pick the **actual Discord channel or category** you want LifeLine to classify/edit.",
    components: [
...
async function editWorldLocationFromDiscordChannel(interaction) {
  ...
  // If World Setup has not saved this channel yet, save it now automatically.
...
async function reviewWorld(interaction) {
  ...
  const { data, error } = await supabase
    .from("npc_world_locations")
    .select("id,name,location_type,discord_object_type,admin_confirmed")
    .eq("guild_id", interaction.guildId)
    .order("name", { ascending: true })
    .limit(25);
  ...
  const picker = new StringSelectMenuBuilder()
    ...
    .addOptions(
      data.map((loc) => ({
        ...
        description:
          `${pretty(loc.discord_object_type)} • ${pretty(loc.location_type)}`.slice(0, 100),
        value: loc.id,
      }))
    );
